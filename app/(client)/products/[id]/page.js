          ...
          <div className="lg:col-span-5">
            <div className="bg-white p-4 sm:p-6 md:p-8 rounded-xl sm:rounded-2xl shadow-sm border border-gray-100">
              ...
              <div className="mb-6 sm:mb-8">
                <h3 className="font-bold text-gray-900 mb-3 sm:mb-4 border-l-4 border-blue-600 pl-2 sm:pl-3 text-sm sm:text-base">
                  Thông số kỹ thuật
                </h3>
                <div className="space-y-2 sm:space-y-3">
                  {Object.entries(SPECS_LABELS).map(([key, label]) => {
                    const value = product.specs?.[key];
                    if (!value) return null;
                    return (
                      <div key={key} className="flex justify-between border-b border-gray-100 pb-2 gap-2">
                        <span className="text-xs sm:text-sm text-gray-500 shrink-0">{label}</span>
